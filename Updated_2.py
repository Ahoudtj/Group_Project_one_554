...
price_train = price_train.drop(columns=['sku'])
...
#price_train = price_train.sample(frac =.001)


#len(np.unique(price_train['sku']))
#len(np.unique(price_train['category']))
...
#price_train.iloc[:, 4:37]



X1 = np.array(price_train.iloc[:, 4:37])
